...
const unsigned char PinDomainID2[3] = {0x53, 0x43, 0x4D};

#define AWP_PIN_DF		"3F004401"
#define AWP_TOKEN_INFO		"3F0044011000"
#define AWP_PUK_FILE		"3F0044012000"
#define AWP_CONTAINERS_MS	"3F0044013000"
#define AWP_OBJECTS_LIST_PUB	"3F0044014000"
#define AWP_OBJECTS_LIST_PRV	"3F0044015000"
#define AWP_OBJECTS_DF_PUB	"3F0044019001"
#define AWP_OBJECTS_DF_PRV	"3F0044019002"
#define AWP_BASE_RSA_PRV	"3F00440190023000"
#define AWP_BASE_RSA_PUB	"3F00440190011000"
#define AWP_BASE_CERTIFICATE	"3F00440190012000"
...
int sc_pkcs15emu_fineid_init_ex(struct sc_pkcs15_card *, struct sc_aid *, struct sc_pkcs15emu_opt *);
...
struct container *Containers2 = NULL;
...
sc_oberthur_get_friends (unsigned int id, struct crypto_container *ccont)
{
	...
	for (cont = Containers2; cont; cont = cont->next)   {
		...
		unsigned char *buff, size_t len, int postpone_allowed)
{
	...
	while (Containers2)   {
		struct container *next = Containers2->next;
	...
	for (offs=0; offs < len;)  {
		...
		if (!Containers2)  {
			...
			cont->next = Containers2;
			Containers2->prev = (void *)cont;
			Containers2 = cont;
		}
		...
		unsigned int file_id, unsigned int size)
{
	...
	kobj.auth_id.len = sizeof(PinDomainID2) > sizeof(kobj.auth_id.value)
			? sizeof(kobj.auth_id.value) : sizeof(PinDomainID2);
	memcpy(kobj.auth_id.value, PinDomainID2, kobj.auth_id.len);
		...
		unsigned int file_id, unsigned int size, int private)
{
	...
	if (private)   {
		dobj.auth_id.len = sizeof(PinDomainID2) > sizeof(dobj.auth_id.value)
				? sizeof(dobj.auth_id.value) : sizeof(PinDomainID2);
		memcpy(dobj.auth_id.value, PinDomainID2, dobj.auth_id.len);
...
sc_pkcs15emu_oberthur_init(struct sc_pkcs15_card * p15card)
{
	...
	if (rv == SC_ERROR_PIN_CODE_INCORRECT)   {
		...
		auth_info.auth_id.len = sizeof(PinDomainID2) > sizeof(auth_info.auth_id.value)
				? sizeof(auth_info.auth_id.value) : sizeof(PinDomainID2);
		memcpy(auth_info.auth_id.value, PinDomainID2, auth_info.auth_id.len);
		auth_info.auth_method	= SC_AC_CHV;
...
oberthur_detect_card(struct sc_pkcs15_card * p15card)
{
	...
	SC_FUNC_CALLED(card->ctx, SC_LOG_DEBUG_VERBOSE);
	if (p15card->card->type != SC_CARD_TYPE_OBERTHUR_FINEID_3)
...
int
sc_pkcs15emu_fineid_init_ex(struct sc_pkcs15_card * p15card, struct sc_aid *aid,
